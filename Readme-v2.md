...
11. **Prometheus Metrics**：暴露积分桶水位、操作计数、事件计数、活跃 NIR 数量等指标
12. **并发控制**：通过 `maxConcurrentActions` 限制同一规则下同时处理的节点数，防止集群容量骤降
13. **Dry-run 模式**：通过 `dryRun: true` 开启，走完所有判定逻辑但不执行操作，仅发送通知，用于上线前验证
...
| `dryRun` | bool | 否 | 开启后走完判定逻辑但不执行操作，仅发送 SNS 通知 |
| `maxConcurrentActions` | int32 | 否 | 同时处理的最大节点数（匹配该规则的），0 表示不限制 |
...
| `METRICS_ADDR` | Prometheus metrics 监听地址 | `:9090` |
...
组件在 `:9090/metrics` 端口暴露 Prometheus 指标，可通过 `METRICS_ADDR` 环境变量自定义地址。

| 指标名称 | 类型 | 标签 | 说明 |
|---------|------|------|------|
| `npd_node_replace_score_bucket_current` | Gauge | `node`, `nodegroup` | 当前节点积分桶中的分数 |
| `npd_node_replace_actions_total` | Counter | `node`, `action`, `escalated` | 执行操作的累计次数 |
| `npd_node_replace_events_total` | Counter | `node`, `event_type` | 记录的 NPD 事件累计次数 |
| `npd_node_replace_nir_active` | Gauge | - | 当前活跃的 NodeIssueReport 资源数量 |

Prometheus scrape 配置示例：
```yaml
- job_name: npd-node-replace
  kubernetes_sd_configs:
    - role: pod
  relabel_configs:
    - source_labels: [__meta_kubernetes_pod_label_app]
      regex: npd-node-replace
      action: keep
